...
test.describe("บัญชีสิ่งของ · เลขทะเบียนอาวุธ", () => {
  ...
  test("กรอกเลขทะเบียนอย่างเดียว แล้วจำนวนอาวุธต้องขึ้นเป็น 1 เอง", async ({ page }) => {
    ...
    // ตั้งแต่ 8 ก.ย. 2569 อาวุธประจำกายมีค่าเริ่มต้นเป็น 1 อยู่แล้ว
    await expect(qty).toHaveValue("1");

    // ล้างช่องเพื่อจำลองผู้ใช้ที่ลบจำนวนออก ซึ่งเป็นทางเดียวที่บั๊กเดิมยังเกิดได้
    await qty.fill("");
    await page.fill("#weapon-serial", serial);

    // ★ ข้อความยืนยันของบั๊กนี้ — ไม่ได้แตะช่องจำนวนอีกเลยแต่ต้องกลับเป็น 1 เอง
    await expect(qty).toHaveValue("1");
