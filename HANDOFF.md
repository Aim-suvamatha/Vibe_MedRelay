...
3. **Prompt 01–10 เสร็จครบ** ยกเว้น CSV export (F7) ที่ตัดไว้ท้ายสุด
   **งานที่ต้องส่งครบแล้วทั้งหมด** เหลือแค่ซ้อม demo และงานที่รอผู้อื่น (§3)
4. **เวลาไม่พอกับแผนเดิมเกินเท่าตัว** ต้องตัดฟีเจอร์ — แผนตัดอยู่ใน §3
5. **สาธิตได้ครบเรื่องแล้วตั้งแต่ต้นทางถึงปลายทาง** เปิดคำขอ → จัดรถ → เดิน 4 ขั้น →
   ส่งมอบ → เปิดทอดถัดไป ทำได้จาก `/track/[caseId]` หน้าเดียว **ยังไม่ต้องมี `/dispatch`**
   ถ้าเวลาหมดตอนนี้ก็ส่งงานได้ — งานที่เหลือคือทำให้เห็นภาพรวม ไม่ใช่ทำให้ใช้งานได้
...
## 2. เสร็จแล้ว — Prompt 01–07 ครบ และ F3 ของ 08
...
migration `0001`–`0016` รันบนโปรเจกต์จริงเรียบร้อย (`0016` รันเมื่อ 6 ก.ย. 2569)

| | บน Supabase จริง | ในเครื่อง (PGlite) |
|---|---|---|
| migration | 0001–0016 | 0001–0016 |
| ตาราง | 11 | 11 |
| ผลทดสอบ | **36/36** (`form_test`) · **11/11** (`rls_test`) · **16/16** (`track_flow_test`) | 11 + 14 + 36 + 16 = **77/77 PASS** |
...
| `src/app/(app)/track/[caseId]/page.tsx` | ✅ ตัวเต็มแล้ว (F3 · Prompt 08) — ดูหัวข้อใน §3 |
...
|---|---|---|---|
| **08** | F2 ศูนย์สั่งการ + F3 ติดตามสถานะ | 6 ชม. | ✅ **เสร็จครบ** (6 ก.ย.) ทั้ง F2 และ F3 · ขึ้น production แล้ว |
| **09** | F4 แดชบอร์ด + F7 export | 4 ชม. | ✅ **แดชบอร์ดเสร็จ** (6 ก.ย.) ขึ้น production แล้ว · F7 ตัดไว้ท้ายสุด |
| **10** | Seed + Deploy | 3 ชม. | ✅ **เสร็จครบ** (6 ก.ย.) deploy + README + demo script |

### ✅ Deploy แล้วและทดสอบบนมือถือจริงผ่าน — https://medrelay-five.vercel.app

**โค้ด `/track` ตัวเต็มขึ้น production แล้วเมื่อ 6 ก.ย. 2569** และเดินครบวงจรบนของจริง
กดปุ่มจริงบน production ด้วย session ของแต่ละบทบาท: จัดรถ → ถึงจุดรับ → ออกเดินทาง
→ ถึงปลายทาง → ส่งมอบ ครบทุกปุ่ม ได้เวลาครบ 6 ค่า เรียงถูกลำดับ เคสปิดเอง รถคืนเป็นว่างเอง
(เคสที่ใช้ตรวจลบทิ้งแล้ว ข้อมูลกลับเป็น 11 เคส · 14 ทอด เท่าเดิม)

⚠️ **แก้โค้ดแล้วต้อง `vercel --prod --yes` ใหม่ทุกครั้ง** ค่า env ถูกฝังตอน build
...
**ลำดับที่แนะนำ — ปรับแล้วหลังทำ F3 เสร็จ 6 ก.ย.**

| ลำดับ | งาน | เวลา | สถานะ |
|---|---|---|---|
| 1 | Deploy ครั้งแรก | ~1 ชม. | ✅ เสร็จ |
| 2 | เติม `/track` ให้ครบวงจร + ปุ่มจัดรถในหน้าเดียวกัน | ~2 ชม. | ✅ เสร็จ · ขึ้น production แล้ว |
| 3 | หน้า Transporter / Receiver / Monitor (รวม F2) | ~1.5 ชม. | ✅ เสร็จ · ขึ้น production แล้ว |
| 4 | `/dashboard` การ์ดตัวเลข | ~1.5 ชม. | ✅ เสร็จ · ขึ้น production แล้ว |
| 5 | README "รันระบบใน 5 นาที" + แก้ demo script | ~1 ชม. | ✅ เสร็จ |

ขั้น 2 เป็นขั้นที่สำคัญที่สุด เพราะ **เวลาทุกค่าที่โครงการนี้อ้างว่าวัดได้ เกิดจากปุ่มในหน้านั้น**
ตอนนี้เดิน `pending → completed` ครบวงจรจากหน้าเดียวได้แล้ว
demo จึงเล่าเรื่องได้ครบตั้งแต่ต้นทางถึงปลายทางโดยยังไม่ต้องมี `/dispatch`
...
### ✅ Prompt 09 — แดชบอร์ดเสร็จแล้ว (6 ก.ย. 2569)
...
**UI เสร็จแล้วเมื่อ 6 ก.ย. 2569** เข้าที่ `/dashboard` ผ่านปุ่มในหน้า `/monitor`

| ไฟล์ | หน้าที่ |
|---|---|
| `migrations/0017_metrics_seconds.sql` | เพิ่มคอลัมน์วินาทีให้ทั้งสอง view |
| `src/lib/duration.ts` | สูตร `median()` และ `formatDuration()` — ไม่ import อะไรเลย |
| `src/lib/duration.test.ts` | 11 ข้อ รันด้วย `npm run test:unit` |
| `src/lib/metrics.ts` | ดึงและรวมตัวเลขทั้งหมด |
| `src/app/(app)/dashboard/page.tsx` | การ์ด 6 ใบ + การกระจาย |

**ตรวจแล้วว่าเลขบนจอตรงกับ `percentile_cont` ของ PostgreSQL ทุกตัว**
รอจัดรถ 510 วิ · รวมต่อทอด 3960 วิ · รวมทั้งเคส 4560 วิ · จาก 12 ทอด 9 เคส

**สี่ข้อที่ต้องรู้ก่อนแก้หน้านี้**

1. **ทำไมต้องมีคอลัมน์วินาที ทั้งที่มีคอลัมน์ `interval` อยู่แล้ว**
   PostgREST คืน interval เป็นสตริงของ PostgreSQL ซึ่ง **เปลี่ยนรูปแบบเมื่อข้าม 24 ชั่วโมง**
   (`'02:03:04'` กลายเป็น `'1 day 02:03:04'`) parser ง่ายๆ ฝั่งเว็บจะอ่านได้ 0 โดยไม่มีใครรู้
   แดชบอร์ดที่แสดงเลขผิดเงียบๆ แย่กว่าแดชบอร์ดที่ไม่มีเลข
2. **`security_invoker = on` ต้องระบุซ้ำทุกครั้งที่ `create or replace view`**
   ถ้าลืม view จะกลับไปรันด้วยสิทธิ์ `postgres` ซึ่งข้าม RLS ทั้งหมด
   แดชบอร์ดจะกลายเป็นช่องรั่วที่ใหญ่ที่สุดในระบบทันที — ตรวจซ้ำได้ด้วย `pg_options_to_table`
3. **ห้ามปัดวินาทีทิ้งในช่วงต่ำกว่า 1 ชั่วโมง**
   มัธยฐานรอจัดรถจริงคือ 510 วินาที ฉบับแรกผมปัดเป็นนาทีแล้วได้ "9 นาที"
   ทั้งที่ค่าจริงคือ 8 นาที 30 วินาที ตัวเลขนี้เป็นข้อพิสูจน์หลักที่โครงการเอาไปอ้าง
   เลขบนจอที่ไม่ตรงกับเลขในเอกสารเป็นเรื่องที่อธิบายยากมากตอนถูกถาม
4. **ใช้ median ไม่ใช่ mean** และ empty state ต้องบอกตรงๆ ว่ายังไม่มีข้อมูล
   **ห้ามแสดงเลข 0** ที่ทำให้เข้าใจผิดว่าวัดแล้วได้ 0 — `median()` จึงคืน `null` ไม่ใช่ `0`

⚠️ **`src/types/database.ts` แก้ด้วยมือในรอบนี้** เพราะ `gen-types.mjs` ต้องใช้
Personal Access Token ซึ่งไม่ควรวางในแชทกับ AI คอลัมน์ที่เพิ่มตรงกับ view จริงแล้ว
(ตรวจด้วย query จริงบนฐานข้อมูล) แต่ควรรัน `node supabase/scripts/gen-types.mjs`
ทับอีกครั้งเมื่อสะดวก เพื่อให้ไฟล์กลับไปเป็นผลลัพธ์ที่ generate ล้วน
...
### ✅ F3 `/track/[caseId]` เสร็จแล้ว (6 ก.ย. 2569) — เดินครบวงจรและทดสอบบนของจริงแล้ว

| ไฟล์ | หน้าที่ |
|---|---|
| `src/lib/leg-flow.ts` | **แหล่งความจริงเดียว**ของลำดับ 6 ขั้น · `nextStep()` `previousStatusOf()` `LEG_STATUS_LABEL` |
| `src/app/(app)/track/[caseId]/page.tsx` | ดึงเคส + ทอด + ผลประเมิน + ตัวเลือกจัดรถ ฝั่ง server |
| `src/app/(app)/track/[caseId]/actions.ts` | `dispatchLeg` · `advanceLeg` · `startNextLeg` |
| `src/app/(app)/track/[caseId]/leg-card.tsx` | เส้นเวลา 6 ขั้น + ปุ่มของขั้นถัดไป + ฟอร์มส่งมอบ |
| `src/app/(app)/track/[caseId]/next-leg-form.tsx` | ปุ่มเปิดทอดถัดไป |
| `supabase/migrations/0016_vehicle_status_sync.sql` | trigger จองรถ/คืนรถตามสถานะทอด |
| `supabase/tests/track_flow_test.sql` | 16 ข้อ เดินตามลำดับเดียวกับที่ Server Action ยิงจริง |

**ทดสอบแล้วสองชั้น ไม่ใช่แค่ compile ผ่าน**

1. `track_flow_test.sql` — **16/16 PASS** บนฐานข้อมูลจริง สวมบทบาททั้ง 4 บัญชี
2. กดปุ่มจริงผ่าน Server Action บน dev server ด้วย session ของแต่ละบทบาท
   เปิดคำขอ → จัดรถ → ถึงจุดรับ → ออกเดินทาง → ถึงปลายทาง → ส่งมอบ → เปิดทอดที่ 2 → จัดรถทอด 2
   ครบทุกปุ่ม ได้เวลาครบ 6 ค่า เรียงถูกลำดับ และเคสปิด/เปิดเองถูกต้อง
   (เคสที่ใช้ตรวจถูกลบทิ้งแล้ว สถานะรถคืนค่าเดิมแล้ว)

**หกข้อที่ตัดสินใจไว้ อย่าเปลี่ยนโดยไม่อ่านเหตุผล**

1. **แสดงปุ่มของขั้นถัดไปขั้นเดียว** ไม่ใช่ทั้ง 6 ปุ่ม — `leg_time_order` ปฏิเสธการข้ามขั้นอยู่แล้ว
   ปุ่มที่กดแล้วพังแน่ๆ ไม่ควรมีบนหน้าจอ ผู้ใช้ในสนามไม่มีเวลามาเดาว่าปุ่มไหนกดได้
2. **ปุ่มจัดรถอยู่ในหน้านี้ด้วย** จึงเดิน `pending → completed` ครบวงจรจากหน้าเดียว
   demo เล่าเรื่องได้ครบโดยยังไม่ต้องมี `/dispatch`
3. **`transporter_id` ถูกตั้งตอนจัดรถ** เป็นกุญแจที่ทำให้ชุดลำเลียงกดขั้นถัดไปได้
   policy `leg_update` ยอมคนที่เป็น `transporter_id` — ถ้าไม่ตั้ง ทอดจะเดินต่อไม่ได้เลย
4. **ปุ่มส่งมอบมีรายการตรวจ ทบ.466-903** (`docs_ok` `property_ok` `missing_note`)
   บันทึกทุกครั้งแม้ติ๊กครบ เพราะ "ตรวจแล้วครบ" กับ "ไม่เคยตรวจ" ต่างกันในทางคดี
5. **`receiver_id` ตั้งให้เฉพาะเมื่อคนกดสังกัดหน่วยปลายทางของทอดนั้น**
   ถ้าชุดลำเลียงเป็นคนกด ปล่อยว่างไว้ดีกว่าใส่ผิดคน — ช่องลงนามผู้รับในกระดาษก็เว้นว่างได้
6. **ใช้ `<details>` ของ HTML แท้** สำหรับพับเส้นเวลาและฟอร์มทอดถัดไป
   ไม่ต้องพึ่ง JavaScript · screen reader หาเจอแม้ยังพับอยู่ · ทดสอบด้วยการ POST ตรงได้

### ✅ หน้า Transporter / Receiver / Monitor เสร็จแล้ว (6 ก.ย. 2569)

เดิมสามหน้านี้เป็นโครงเปล่าที่เขียนว่า "หน้านี้จะสร้างใน F3 · Prompt 08"
ปุ่มล่างจึงพาไปทางตัน ชุดลำเลียงหาภารกิจตัวเองจากในแอปไม่ได้เลย

| ไฟล์ | หน้าที่ |
|---|---|
| `src/lib/leg-queries.ts` | select string + mapper ที่ทั้งสามหน้าใช้ร่วมกัน |
| `src/components/medrelay/leg-list.tsx` | `LegCardLink` · `LegSection` การ์ดรายการทอด |
| `src/components/medrelay/no-access.tsx` | ข้อความเดียวกันทุกหน้าเมื่อไม่มีบทบาท |
| `/transporter` | ภารกิจที่จ่ายให้ผู้ใช้คนนี้ ไม่ใช่ของทั้งหน่วย |
| `/receiver` | ผู้ป่วยที่กำลังเดินทางมาหน่วยนี้ |
| `/monitor` | คิวรอจัดรถ + กำลังเดินทาง + กระดานรถ (**นี่คือ F2**) |

**สามข้อที่ตัดสินใจไว้**

1. **การ์ดทั้งใบเป็นลิงก์ไป `/track`** ไม่ใช่ปุ่มเล็กมุมขวา
   ผู้ใช้กดด้วยนิ้วโป้งขณะสวมถุงมือ เป้าหมายเล็กกว่านี้จะกดพลาด
   และไม่มีปุ่มลัดเปลี่ยนสถานะบนรายการ เพราะปุ่มที่กดได้โดยไม่เห็นบริบท
   คือปุ่มที่กดผิดเคสได้ง่ายที่สุด
2. **การ์ดบอก "ขั้นถัดไปคืออะไร" ไม่ใช่แค่สถานะปัจจุบัน**
   คนเปิดหน้ารายการมาถามว่าต้องทำอะไรต่อ ไม่ได้มาถามว่าตอนนี้อยู่ขั้นไหน
3. **`/monitor` ไม่ใช้ Realtime** ตามที่ §3 ระบุให้ตัด เป็น server component ล้วน
   ถ้าจะเติมทีหลัง ต้องมีตัวบอกบนจอด้วยว่าเชื่อมต่ออยู่หรือหลุดแล้ว
   realtime ที่หลุดเงียบๆ อันตรายกว่าไม่มี realtime เลย

**⚠ บทเรียนเรื่องการกันสิทธิ์ — อ่านก่อนเขียนหน้าใหม่ทุกหน้า**

รอบแรกผมเขียนหน้าพวกนี้แบบ "ดึงข้อมูลก่อน แล้วเอา `RoleGate` ครอบผลลัพธ์"
ตรวจแล้วพบว่า transporter ที่พิมพ์ URL `/monitor` เข้ามาตรงๆ
**ได้ข้อมูลรถของหน่วยตัวเองส่งไปถึงเครื่องเรียบร้อยแล้ว ทั้งที่จอไม่แสดง**

RLS กันรถหน่วยอื่นไว้อยู่แล้วจึงไม่ใช่ข้อมูลที่เขาไม่มีสิทธิ์เห็น
แต่ขัดกฎที่เขียนไว้เองในคอมเมนต์ของ `role-gate.tsx` ตรงๆ ว่า
"ถ้าไม่ควรเห็น ต้องไม่ถูกส่งมาตั้งแต่แรก"

แก้ด้วย `hasAnyRole()` ใน `src/lib/auth/profile.ts` ซึ่งรันฝั่ง server
จึงใช้ตัดสินได้ว่า **จะยิง query หรือไม่ยิง** ต่างจาก `RoleGate` ที่รันบน browser
และแค่ "ไม่วาด" ของที่ส่งไปถึงเครื่องแล้ว

**กฎ** — หน้าไหนที่ดึงข้อมูลเฉพาะบทบาท ต้องเช็ค `hasAnyRole()` **ก่อน** ยิง query เสมอ
`RoleGate` ใช้ได้เฉพาะกับปุ่มหรือทางลัดที่ไม่ได้พ่วงข้อมูลมาด้วย (เช่นปุ่มจัดรถใน `leg-card.tsx`)

### 🎯 งานถัดไป — เรียงตามลำดับที่แนะนำ

**งานเขียนโค้ดที่ต้องส่งเสร็จครบแล้ว** เหลือสามอย่างที่ไม่ใช่การเขียนฟีเจอร์

| ลำดับ | งาน | เวลา | หมายเหตุ |
|---|---|---|---|
| 1 | **ซ้อม demo จริงตามสคริปต์** | ~40 นาที | จับเวลาให้ได้ 4 นาที · อัดวิดีโอสำรองไว้เผื่อเน็ตล่ม |
| 2 | รัน `gen-types.mjs` ทับ | ~5 นาที | `database.ts` แก้ด้วยมือรอบที่แล้ว ดูหัวข้อ Prompt 09 |
| 3 | CSV export (F7) | ~1 ชม. | หยิบมาทำเฉพาะถ้าเหลือเวลา · `v_casualty_report` พร้อมแล้ว |

`/dispatch` แยกต่างหาก **ไม่ต้องทำแล้ว** — คิวคำขอกับกระดานรถอยู่ใน `/monitor` เรียบร้อย

**⚠ ก่อนขึ้นเวทีต้องอ่าน `docs/demo-script.md` ฉบับใหม่**
ฉบับเดิมอ้าง `/dispatch` ซึ่งไม่มีอยู่จริง และเขียนบทพูดว่าคำขอ "ขึ้นเองครับ"
ทั้งที่ตัด realtime ออกไปแล้ว **ระบบต้องรีเฟรชหน้าจอเอง**
ถ้าพูดตามบทเดิมจะต้องแก้ตัวกลางเวที — แก้ให้ตรงกับระบบจริงแล้วเมื่อ 6 ก.ย.

**สาธิตได้ครบแล้วตั้งแต่ตอนนี้ โดยไม่ต้องพิมพ์ URL เองสักครั้ง**
`/sender` เปิดคำขอ → `/monitor` เห็นคิวแล้วกดจัดรถ → `/transporter` เห็นภารกิจแล้วเดินสถานะ
→ `/receiver` เห็นผู้ป่วยกำลังมาแล้วกดรับมอบ ทุกหน้าเข้าถึงได้จากปุ่มล่าง

⚠ ข้อ 2 ใช้ **median ไม่ใช่ mean** และ empty state ต้องบอกตรงๆ ว่ายังไม่มีข้อมูล
**ห้ามแสดงเลข 0** ที่ทำให้เข้าใจผิดว่าวัดแล้วได้ 0

### กติกาที่ห้ามผิดตอนเขียนหน้าที่เหลือ
...
- **ห้ามยิง update ตารางอื่นแทนผู้ใช้โดยไม่ตรวจว่าเขามี policy ให้แก้จริง** — §5 ข้อ 13
  UPDATE ที่ RLS ปฏิเสธจะแก้ 0 แถวเงียบๆ ไม่มี error ให้เห็น ต้องทำด้วย trigger แทน
- **ลำดับสถานะทอดอ่านจาก `src/lib/leg-flow.ts` เท่านั้น** ห้ามเขียนลำดับซ้ำในหน้าใหม่
  ไฟล์นั้นสะท้อน constraint `leg_time_order` โดยตรง ถ้าแยกกันเขียนจะเพี้ยนกันวันใดวันหนึ่ง
- component พร้อมใช้: `TriageDot` `TriageChip` `PrecedenceBadge` `RelativeTime`
  ...
  และ `src/app/(app)/track/[caseId]/leg-card.tsx`
...
```bash
node supabase/scripts/sql.mjs --no-tx supabase/tests/form_test.sql        # 36/36
node supabase/scripts/sql.mjs --no-tx supabase/tests/rls_test.sql         # 11/11
node supabase/scripts/sql.mjs --no-tx supabase/tests/track_flow_test.sql  # 16/16
```
...
```bash
npm run test:unit                                        # สูตร median/เวลา 11 ข้อ
npm run db:test                                          # PGlite ในเครื่อง 77 ข้อ
node supabase/scripts/sql.mjs --no-tx supabase/tests/form_test.sql        # ของจริง 36 ข้อ
node supabase/scripts/sql.mjs --no-tx supabase/tests/track_flow_test.sql  # ของจริง 16 ข้อ
...
## 5. ⚠️ สิบสามข้อที่จะเสียเวลาถ้าไม่รู้ล่วงหน้า
...
**13. UPDATE ที่ RLS ปฏิเสธ "แก้ 0 แถว" เงียบๆ ไม่ใช่ error** (เจอเมื่อ 6 ก.ย. 2569)

เจอตอนทดสอบ `/track` แบบ end-to-end จนจบ — ปุ่มทุกปุ่มขึ้นว่าสำเร็จ
แต่รถค้างสถานะ `dispatched` ตลอดไปและหายจากรายการรถว่าง

สาเหตุ: ตอนกดส่งมอบ โค้ดยิง `update vehicle set status='available'` ต่อท้าย
คนที่กดคือชุดลำเลียงหรือผู้รับปลายทาง ซึ่ง policy `vehicle_update` ยอมเฉพาะ monitor/admin
PostgREST ตอบ 200 พร้อม `error: null` เพราะ **ไม่มีแถวไหนผ่าน USING ของ policy ให้แก้เลย**
ซึ่งไม่ใช่ความผิดพลาด แค่ "ไม่มีอะไรตรงเงื่อนไข"

⚠ ต่างจาก INSERT ที่ล้มด้วย `42501` ให้เห็น — **UPDATE ไม่มีสัญญาณอะไรเลย**
หลังสาธิตครบหนึ่งรอบจะไม่มีรถให้จัดในรอบถัดไป โดยไม่มี error ที่ไหนบอก

**แก้ด้วย trigger `security definer` ไม่ใช่ด้วยการเปิด policy ให้กว้างขึ้น**
`0016_vehicle_status_sync.sql` แตะได้เฉพาะรถที่ผูกกับทอดที่กำลังเปลี่ยนสถานะ
ส่วนการเปิด policy จะเท่ากับยอมให้ใครก็ได้เปลี่ยนสถานะรถคันไหนก็ได้ในระบบ
เป็นวิธีเดียวกับที่ `sync_case_status` ใน `0009` ใช้มาตั้งแต่ต้น

**กฎที่ได้จากข้อนี้** — เขียน update ข้ามตารางแทนผู้ใช้เมื่อไร ให้ถามก่อนว่า
"ผู้ใช้คนนี้มี policy ให้แก้ตารางนั้นจริงไหม" ถ้าไม่ใช่ ต้องทำด้วย trigger
และต้องมีข้อทดสอบที่สวมบทบาทเป็น **คนที่ไม่มีสิทธิ์** กำกับไว้ (`track_flow_test.sql` ข้อ K15)
